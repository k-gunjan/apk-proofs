...
struct Approval {
    comm: KeysetCommitment<BigCurveCongig>,
    sig: Signature,
    pk: PublicKey,
}

thread_local! {
    static CACHE: RefCell<Option<KeysetCommitment<BigCurveCongig>>> = RefCell::new(None);
}
...
fn get_keyset_commitment<F>(f: F) -> KeysetCommitment<BigCurveCongig>
where
    F: FnOnce() -> KeysetCommitment<BigCurveCongig>,
{
    CACHE.with(|cell| {
...
impl Validator {
    ...
    fn approve(
        ...
        let new_validator_set_commitment = get_keyset_commitment(|| {
            Keyset::<BigCurveCongig>::new(new_validator_set.raw_public_keys()).commit(kzg_pk)
        });
...
struct LightClient {
    ...
    current_validator_set_commitment: KeysetCommitment<BigCurveCongig>,
    quorum: usize,
}
...
impl LightClient {
    fn init(
        kzg_vk: RawKzgVerifierKey<BW6_761>,
        genesis_keyset_commitment: KeysetCommitment<BigCurveCongig>,
        quorum: usize,
    ) -> Self {
        Self {
            kzg_vk,
    ...
    fn verify_aggregates(
        ...
        new_validator_set_commitment: KeysetCommitment<BigCurveCongig>,
    ) {
...
impl TrustlessHelper {
    fn new(
        ...
        genesis_validator_set_commitment: &KeysetCommitment<BigCurveCongig>,
        kzg_params: URS<BW6_761>,
    ) -> Self {
        let prover = Prover::new(
            Keyset::<BigCurveCongig>::new(genesis_validator_set.raw_public_keys()),
    ...
    fn aggregate_approvals(
        ...
        SimpleProof,
        Signature,
        KeysetCommitment<BigCurveCongig>,
    ) {
        let t_approval = start_timer!(|| {
        ...
        self.prover = Prover::new(
            Keyset::<BigCurveCongig>::new(new_validator_set.raw_public_keys()),
...
fn hash_commitment(commitment: &KeysetCommitment<BigCurveCongig>) -> G2Projective {
...
fn main() {
    ...
    let keyset = Keyset::<BigCurveCongig>::new(genesis_validator_set.raw_public_keys());
