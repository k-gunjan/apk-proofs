...
use apk_proofs::instances::bls12_377_bw6_761::kzg::{KeysetCommitment377, Pcs, Prover377, SimpleProof377, Verifier377};
use apk_proofs::instances::bls12_377_bw6_761::{AccountablePublicInput377, Keyset377, OuterScalar};
...
struct Approval {
    comm: KeysetCommitment377,
    sig: Signature,
    pk: PublicKey,
}

thread_local! {
    static CACHE: RefCell<Option<KeysetCommitment377>> = RefCell::new(None);
}
...
fn get_keyset_commitment<F>(f: F) -> KeysetCommitment377 where F: FnOnce() -> KeysetCommitment377 {
    CACHE.with(|cell| {
...
impl Validator {
    ...
    fn approve(&self, new_validator_set: &ValidatorSet, kzg_pk: &KzgCommitterKey<ark_bw6_761::G1Affine>) -> Approval {
        ...
        let new_validator_set_commitment = get_keyset_commitment(|| {
            Keyset377::new(new_validator_set.raw_public_keys()).commit::<Pcs>(kzg_pk)
        });
...
struct LightClient {
    ...
    current_validator_set_commitment: KeysetCommitment377,
    quorum: usize,
}
...
impl LightClient {
    fn init(
        kzg_vk: RawKzgVerifierKey<BW6_761>,
        genesis_keyset_commitment: KeysetCommitment377,
        quorum: usize,
    ) -> Self {
        Self {
            kzg_vk,
    ...
    fn verify_aggregates(&mut self,
                         public_input: AccountablePublicInput377,
                         proof: &SimpleProof377,
                         aggregate_signature: &Signature,
                         new_validator_set_commitment: KeysetCommitment377) {
        ...
        let verifier = Verifier377::new(self.kzg_vk.clone(), self.current_validator_set_commitment.clone(), Transcript::new(b"apk_proof"));
...
impl TrustlessHelper {
    fn new(genesis_validator_set: ValidatorSet, genesis_validator_set_commitment: &KeysetCommitment377, kzg_params: URS<BW6_761>) -> Self {
    ...
    fn aggregate_approvals(&mut self, new_validator_set: ValidatorSet, approvals: Vec<Approval>) -> (AccountablePublicInput377, SimpleProof377, Signature, KeysetCommitment377) {
...
fn hash_commitment(commitment: &KeysetCommitment377) -> G2Projective {
...
fn main() {
    ...
    let kzg_params = setup::generate_for_domain::<_, OuterScalar, Pcs>(log_n as u32, rng);
    end_timer!(t_setup);
    ...
    let keyset = Keyset377::new(genesis_validator_set.raw_public_keys());
    let genesis_validator_set_commitment = keyset.commit::<Pcs>(&kzg_params.ck());
    end_timer!(t_genesis);
